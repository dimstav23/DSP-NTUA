...
for i = 1:size(y,2)
    ...
    %vriskw mesw ths synarthshs ltp analysis ttous syntelestes tou filter
    [b(i,:),M(i)] = ltp_analysis(lpc_error);
end
...
bi_Quantized=linearQ(b,5);
Mi_Quantized=10.^(linearQ(log10(M),5));
...
for i = 1:size(y,2)
    ...
    b_Quantized(i,1) = 1;
	b_Quantized(i,(2:M(i))) = 0;
	b_Quantized(i,(M(i)+1)) = (-1)*bi_Quantized(i); 
end
%kanw syn8esh ths fwnhs mesw mias elafrws allagmenhs synarthshs vocoder
synthesized_voice = Quantized_Values_Celp_Voc(279563, Fs ,0.030*48000,0.020*48000,Gain_Quantized,ai_Quantized,b_Quantized,583,s);
